...
# Implemenation of BackSpace Key Binding(Removes the last character from entry widget)
def bspace_press(event):
    x = e.get()
    x = x[:-1]
    e.delete(0, END)
    e.insert(0, x)
...
root.bind("<BackSpace>", bspace_press)
root.bind()
root.iconbitmap('1.5.ico')
...
button_Clear = Button(root, text = "C", width = 6, height = 2, font = ('arial', 20, 'bold'), bd = 4, bg = "gold3",
                      command = button_clear)
button_Clear.grid(row = 1, column = 0, pady = 1)
button_AllClear = Button(root, text = "CE", width = 6, height = 2, font = ('arial', 20, 'bold'), bd = 4, bg = "gold3",
                         command = button_eclear)
